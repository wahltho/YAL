...
function P.YalinitGlobal()
    ...
    speedrstr250cmdtable = {"des", "del", "clr", "2", "5", "0", "slash", "3L", "exec", "end"}

    setilsstepindex = 1
    setilsstepindexprevious = 1
    setilssteprepeat = false
    setilsisils = "XXX"
    setilshasdme = false
    ...
    procedurelock = NOPROCEDURE
    procedurelock2= NOPROCEDURE

    procedureabort = false
    procedureskipstep = false

    procedurestepindex = 1
    procedurestepindexprevious = 1
    proceduresteprepeat = false

    procedurestepindex2 = 1
    procedurestepindexprevious2 = 1
    proceduresteprepeat2 = false
...
function P.initDataref()
    ...
    if helpers.isXp11 then
        ...
        baroaircraftinhg = globalProperty("sim/weather/barometer_current_inhg") -- currently unused
    end
    if helpers.isXp12 then
        baroregionpas = globalProperty("sim/weather/region/qnh_pas")
        baroaircraftpas = globalProperty("sim/weather/aircraft/qnh_pas") -- currently unused
    end
...
function aircraftonrwy(aircraftlat, aircraftlon, rwystartlat, rwystartlon, rwyendlat, rwyendlon, dist)
    ...
    rwystartlatrad = math.rad(rwystartlat)
    rwystartlonrad = math.rad(rwystartlon)
    rwyendlatrad = math.rad(rwyendlat)
    rwyendlonrad = math.rad(rwyendlon)
    aircraftlatrad = math.rad(aircraftlat)
    aircraftlonrad = math.rad(aircraftlon)

    v1 = (rwyendlonrad - rwystartlonrad) * math.cos(rwystartlatrad)
    v2 = (rwyendlatrad - rwystartlatrad)
    d1 = (aircraftlatrad - rwystartlatrad)
    d2 = (aircraftlonrad - rwystartlonrad) * math.cos(rwystartlatrad)
    s = d1 * v1 + d2 * v2

    disttorwy = math.sqrt(math.abs(d1 ^ 2 + d2 ^ 2 - 2 * s))
...
--------------------------------------------------------------------------------------------------------------
function calclocalqnhinch()

    if helpers.isXp11 then
        localqnhinch = get(baroregioninhg)
        localqhnhpas = roundnumber(convqnhinchtopas(localqnhinch) / 100)
    end

    if helpers.isXp12 then
        localqhnhpas = roundnumber(get(baroregionpas) / 100)
        localqnhinch = roundnumber((localqhnhpas * PASTOINCH), 2)
    end
    return (localqnhinch)
...
--------------------------------------------------------------------------------------------------------------
function convqnhinchtopas(baroinch)

    qnhpas = roundnumber(baroinch * INCHTOPAS)
...
function mastercaution()
    ...
    if ((procedurelock ~= NOPROCEDURE) and (configvalues[CONFIGVOICEADVICEONLY] == ON)) then
...
function speedrstr250()

    helpers.command_once("laminar/B738/button/fmc1_des")

    local FMC1Line00L = helpers.get("laminar/B738/fmc1/Line00_L")
    local FMC1Line03L = helpers.get("laminar/B738/fmc1/Line03_L")

    local tableindex = 1

    if ((string.len(FMC1Line00L) < 9) or ((string.sub(FMC1Line00L, 6, 9) ~= "ECON") and (string.sub(FMC1Line00L, 2, 4) ~= "ACT"))) then
        return false
    end

    if ((string.sub(FMC1Line03L, 1, 3) ~= "250") and (string.sub(FMC1Line03L, 1, 3) ~= "   ") and
        ((string.sub(FMC1Line03L, 5, 9) == "FL100") or (string.sub(FMC1Line03L, 5, 9) == "10000"))) then
        while (speedrstr250cmdtable[tableindex] ~= "end") do
            sasl.logDebug("while loop speedrstr250")
            commandtableentry(COMMAND, "laminar/B738/button/fmc1_" .. speedrstr250cmdtable[tableindex])
            tableindex = tableindex + 1
        end
        commandtableentry(TEXT, "Descent Started")
        commandtableentry(TEXT, "Speed Restriction 250 below 10000 feet set")
    end
...
function speedrstr250proc()

    if (procedurelock ~= NOPROCEDURE) then
        return true
    else
        procedurelock = SETSPDRESTR250PROCEDURE
    end

    if (flightstate <= 2) then
        procedurelock = NOPROCEDURE
        commandtableentry(TEXT, "Set Speed Restriction 250 aborted")
        return true
    end
...
function speedrstr250proc_(phase)
    if phase == SASL_COMMAND_BEGIN then
        speedrstr250proc()
    end
    return 0
...
my_command_speedrstr250 = sasl.createCommand(definitions.APPNAMEPREFIX .. "/speedrstr250", "Set Speed Restriction 250")
sasl.registerCommandHandler(my_command_speedrstr250, 0, speedrstr250proc_)
--sasl.appendMenuItem(P.menu_main, "Set Speed Restriction 250", speedrstr250proc)
...
function setilssteps()
    ...
    if (setilsstepindex == 1) then
        helpers.command_once("laminar/B738/button/fmc1_init_ref")
        if ((string.len(FMC1Line00L) < 9) or (string.sub(FMC1Line00L, 7, 9) ~= "APP")) then
            setilsstepindex = setilsstepindex - 1
        end
    end

    if (setilsstepindex == 2) then
        if ((string.len(FMC1Line00L) < 9) or (string.sub(FMC1Line00L, 7, 9) ~= "APP")) then
            helpers.command_once("laminar/B738/button/fmc1_init_ref")
            setilsstepindex = setilsstepindex - 1
        else
            if ((string.len(FMC1Line04X) == 24) and (string.len(FMC1Line04L) == 24)) then
                setilsisils = string.sub(FMC1Line04X, 2, 4)

                if ((setilsisils == "ILS") or (setilsisils == "GLS")) then
                    slashpos = string.find(FMC1Line04L, "/")
                    if not ((slashpos ~= nil) and (slashpos < 13)) then
                        setilsisils = "XXX"
                    end
                end

                if (setilsisils == "ILS") then
                    if (configvalues[CONFIGVOICEADVICEONLY] == ON) then
                        commandtableentry(ADVICE, "I L S Approach")
                    else
                        commandtableentry(TEXT, "I L S Approach")
                    end
                    apphasils = true
                elseif ((setilsisils == "GLS") and (get(mmrinstalled) == ON)) then
                    if (configvalues[CONFIGVOICEADVICEONLY] == ON) then
                        commandtableentry(ADVICE, "G L S Approach")
                    else
                        commandtableentry(TEXT, "G L S Approach")
                    end
                    apphasils = true
                elseif (setilsisils == "XXX") then
                    if (configvalues[CONFIGVOICEADVICEONLY] == ON) then
                        commandtableentry(ADVICE, "No I L S or G L S Frequency, check APPROACH page")
                    else
                        commandtableentry(TEXT, "No I L S or G L S Frequency, check APPROACH page")
                    end
                    apphasils = false
                    setilsstepindex = 8
                elseif ((setilsisils ~= "ILS") and (setilsisils ~= "GLS")) then
                    if (configvalues[CONFIGVOICEADVICEONLY] == ON) then
                        commandtableentry(ADVICE, "No I L S or G L S Approach")
                    else
                        commandtableentry(TEXT, "No I L S or G L S Approach")
                    end
                    apphasils = false
                    setilsstepindex = 8
                end
    ...
    if (setilsstepindex == 3) then
        if ((string.len(FMC1Line00L) < 9) or (string.sub(FMC1Line00L, 7, 9) ~= "APP")) then
            helpers.command_once("laminar/B738/button/fmc1_init_ref")
            setilsstepindex = setilsstepindex - 1
        else
            if (setilsisils == "ILS") then
                nav1freqnew = tonumber(string.sub(FMC1Line04L, 1, slashpos - 1)) * 100
                if (nav1freqnew ~= get(nav1freq)) then
                    if (configvalues[CONFIGVOICEADVICEONLY] == ON) then
                        commandtableentry(ADVICE, "Set I L S Frequency " .. tostring(nav1freqnew / 100))
                        setilsstepindex = setilsstepindex - 1
                    else
                        if (get(mmrinstalled) == ON) then
                            if ((get(mmrcptactvalue) ~= get(nav1freq)) or (get(mmrcptactmode) ~= MMRILS)) then
                                setmmrils(MMRCAPTAIN, get(nav1freq))
                            end
                        else
                            set(nav1stdbyfreq, get(nav1freq))
                            set(nav1freq, nav1freqnew)
                        end
                    end
                elseif ((configvalues[CONFIGVOICEADVICEONLY] == ON) and not setilssteprepeat) then
                    commandtableentry(ADVICE, "I L S Frequency checked and " .. tostring(nav1freqnew / 100))
                end
            elseif ((setilsisils == "GLS") and (get(mmrinstalled) == ON)) then
                glsfreqnew = tonumber(string.sub(FMC1Line04L, 1, 5))
                if ((get(mmrcptactvalue) ~= glsfreqnew) or (get(mmrcptactmode) ~= MMRGLS)) then
                    if (configvalues[CONFIGVOICEADVICEONLY] == ON) then
                        commandtableentry(ADVICE, "Set G L S Channel " .. glsfreqnew)
                        setilsstepindex = setilsstepindex - 1
                    else
                        setmmrgls(MMRCAPTAIN, glsfreqnew)
                    end
                elseif ((configvalues[CONFIGVOICEADVICEONLY] == ON) and not setilssteprepeat) then
                    commandtableentry(ADVICE, "G L S Channel checked and " .. glsfreqnew)
                end
            end
        end
    end

    if (setilsstepindex == 4) then
        if ((string.len(FMC1Line00L) < 9) or (string.sub(FMC1Line00L, 7, 9) ~= "APP")) then
            helpers.command_once("laminar/B738/button/fmc1_init_ref")
            setilsstepindex = setilsstepindex - 1
        else
            mcppilotcoursenew = tonumber(string.sub(FMC1Line04L, slashpos + 1, slashpos + 3))

            if (get(mcppilotcourse) ~= mcppilotcoursenew) then
                if (configvalues[CONFIGVOICEADVICEONLY] == ON) then
                    commandtableentry(ADVICE, "Set Course " .. mcppilotcoursenew)
                    setilsstepindex = setilsstepindex - 1
                else   
                    set(mcppilotcourse, mcppilotcoursenew)
                    if ((ilshasdme == ON) and (get(mcpcopilotcourse) ~= mcppilotcoursenew)) then
                        set(mcpcopilotcourse, mcppilotcoursenew)
                    end  
                end
            elseif ((configvalues[CONFIGVOICEADVICEONLY] == ON) and not setilssteprepeat) then
                commandtableentry(ADVICE, "Course checked and " .. mcppilotcoursenew)
            end 
        end                
    end

    if (setilsstepindex == 5) then
        if (setilsisils == "ILS") then
            setilshasdme = (helpers.get("laminar/radios/pilot/nav_has_dme") == ON)
            if setilshasdme then
                if (configvalues[CONFIGVOICEADVICEONLY] == ON) then
                    commandtableentry(ADVICE, "I L S has D M E")
                else
                    commandtableentry(TEXT, "I L S has D M E")
                end
            else
                if (configvalues[CONFIGVOICEADVICEONLY] == ON) then
                    commandtableentry(ADVICE, "I L S has no D M E")
                else
                    commandtableentry(TEXT, "I L S has no D M E")
                end
            end
        end
    end

    if (setilsstepindex == 6) then
        if ((configvalues[CONFIGAUTOFUNCTIONS] == ON) or (procedurelock == SETILSPROCEDURE)) then
            if ((setilsisils == "ILS") and setilshasdme) then
                if (get(nav2freq) ~= get(nav1freq)) then
                    if (get(mmrinstalled) == ON) then
                        if ((get(mmrfoactvalue) ~= get(nav1freq)) or (get(mmrfoactmode) ~= MMRILS)) then
    ...
    if (setilsstepindex == setilsstepindexprevious) then
        setilssteprepeat = true
    else
        setilssteprepeat = false
        setilsstepindexprevious = setilsstepindex
    end
...
--sasl.appendMenuItem(P.menu_main, "Set ILS/GLS Frequency and Course", setilsproc)
...
function setvrefproc()
    ...
    if (procedurelock == NOPROCEDURE) then
        procedurelock = SETVREF30PROCEDURE
    end
...
function setirs(irs, state)
    ...
    local irsleftposdiff = math.abs(get(irsleftpos) - state)
    local irsrightposdiff = math.abs(get(irsrightpos) - state)

    if ((state ~= nil) and (irs ~= nil)) then
        if ((irs == LEFTIRS) or (irs == BOTHIRS)) then
            if (state > get(irsleftpos)) then
                while (irsleftposdiff > 0) do
                    sasl.logDebug("while loop irs L R")
                    helpers.command_once("laminar/B738/toggle_switch/irs_L_right")
                    irsleftposdiff = irsleftposdiff - 1
                end
            elseif (state < get(irsleftpos)) then
                while (irsleftposdiff > 0) do
                    sasl.logDebug("while loop irs L L")
        ...
        if ((irs == RIGHTIRS) or (irs == BOTHIRS)) then
            if (state > get(irsrightpos)) then
                while (irsrightposdiff > 0) do
                    sasl.logDebug("while loop irs R R")
                    helpers.command_once("laminar/B738/toggle_switch/irs_R_right")
                    irsrightposdiff = irsrightposdiff - 1
                end
            elseif (state < get(irsrightpos)) then
                while (irsrightposdiff > 0) do
                    sasl.logDebug("while loop irs R L")
...
function coldanddarksteps()
    ...
    if ((procedurestepindex > 30) or procedureabort) then
        procedurelock = NOPROCEDURE
        procedurestepindex = 1
        procedureabort = false
        return true
    ...
    if (procedurestepindex == 1) then
        if not setview(configvalues[CONFIGVIEWOVERHEADPANEL]) then
            procedurestepindex = procedurestepindex - 1
        end
    end

    if (procedurestepindex == 2) then
    ...
    if (procedurestepindex == 6) then
        if not setview(configvalues[CONFIGVIEWOVERHEADPANEL]) then
            procedurestepindex = procedurestepindex - 1
        end
    end

    if (procedurestepindex == 7) then
        ...
        if ((configvalues[CONFIGUSEGROUNDPOWER] == ON) and (get(gpuavailable) == ON)) then
            if (get(gpuon) == OFF) then
                if (configvalues[CONFIGVOICEADVICEONLY] == ON) then
                    ...
                    procedurestepindex = 19
                    return true
                end
            else
                if ((configvalues[CONFIGVOICEADVICEONLY] == ON) and not proceduresteprepeat) then
    ...
    if (procedurestepindex == 13) then
        if (configvalues[CONFIGVOICEADVICEONLY]  ~= ON) then
            helpers.command_once("laminar/B738/spring_toggle_switch/APU_start_pos_dn")
            commandtableentry(TEXT, "A P U Checked and Started")
        end
    end

    if (procedurestepindex == 14) then
    ...
    if (procedurestepindex == 20) then
        if not setview(configvalues[CONFIGVIEWUPPEROVERHEADPANEL]) then
            procedurestepindex = procedurestepindex - 1
        end
    end

    if (procedurestepindex == 21) then
        if (configvalues[CONFIGVOICEADVICEONLY] ~= ON) then
            setirs(BOTHIRS, IRSNAV)
        end
    end

    if (procedurestepindex == 22) then
    ...
    if (procedurestepindex == 23) then
        if not setview(configvalues[CONFIGVIEWFMS]) then
            procedurestepindex = procedurestepindex - 1
        end
    end

    if (procedurestepindex == 24) then
        if (configvalues[CONFIGVOICEADVICEONLY] == ON) then
    ...
    if (procedurestepindex == 29) then
        if not setview(configvalues[CONFIGVIEWMAINPANEL]) then
            procedurestepindex = procedurestepindex - 1
        end
    end

    if (procedurestepindex == 30) then
        if (configvalues[CONFIGVOICEADVICEONLY] == ON) then
...
function coldanddarkstartup()
    ...
    if (procedurelock ~= NOPROCEDURE) then
        ...
        procedurelock = COLDANDDARKPROCEDURE
    end
...
function apustartupsteps()
    ...
    if ((procedurestepindex > 8) or procedureabort) then
        procedurelock = NOPROCEDURE
        procedurestepindex = 1
        procedureabort = false
        return true
...
function apustartup()
    ...
    if (procedurelock ~= NOPROCEDURE) then
        ...
        procedurelock = APUSTARTUPPROCEDURE
    end
...
function enginestartsteps()
    ...
    if ((procedurestepindex > 34) or procedureabort) then
        procedurelock = NOPROCEDURE
        procedurestepindex = 1
        procedureabort = false
        return true
    ...
    if (procedurestepindex == 33) then
        if not setview(configvalues[CONFIGVIEWMAINPANEL]) then
            procedurestepindex = procedurestepindex - 1
        end
    end

    if (procedurestepindex == 34) then
        if (configvalues[CONFIGVOICEADVICEONLY] == ON) then
...
function enginestart()
    ...
    if (procedurelock ~= NOPROCEDURE) then
        ...
        procedurelock = ENGINESTARTPROCEDURE
    end
...
function engineshutdownsteps()
    ...
    if ((procedurestepindex > 18) or procedureabort) then
        procedurelock = NOPROCEDURE
        procedurestepindex = 1
        procedureabort = false
        return true
    ...
    if (procedurestepindex == 2) then
        if (procedurelock == TURNAROUNDENGINESHUTDOWNPROCEDURE) then
            if ((configvalues[CONFIGUSEGROUNDPOWER] == ON) and (get(gpuavailable) == ON)) then
                if (get(gpuon) == OFF) then
                    if (configvalues[CONFIGVOICEADVICEONLY] == ON) then
                        commandtableentry(ADVICE, "Switch Ground Power On")
        ...
        elseif (procedurelock == FINALENGINESHUTDOWNPROCEDURE) then
            procedurestepindex = 9
            return true
        end
    end
...
function turnaroundengineshutdown()
    ...
    if (procedurelock ~= NOPROCEDURE) then
        ...
        procedurelock = TURNAROUNDENGINESHUTDOWNPROCEDURE
    end
...
function finalengineshutdown()
    ...
    if (procedurelock ~= NOPROCEDURE) then
        ...
        procedurelock = FINALENGINESHUTDOWNPROCEDURE
    end
...
function shutdownsteps()
    ...
    if ((procedurestepindex > 25) or procedureabort) then
        procedurelock = NOPROCEDURE
        procedurestepindex = 1
        procedureabort = false
        return true
...
function shutdown()
    ...
    if (procedurelock ~= NOPROCEDURE) then
        ...
        procedurelock = SHUTDOWNPROCEDURE
    end
...
function teststeps()
    ...
    if ((procedurestepindex > 48) or procedureabort) then
        procedurelock = NOPROCEDURE
        procedurestepindex = 1
        procedureabort = false
        return true
...
function cockpitinitsteps()
    ...
    if ((procedurestepindex > 22) or procedureabort) then
        procedurelock = NOPROCEDURE
        procedureabort = false
        procedurestepindex = 1
        procedureabort = false
    ...
    if ((procedurestepindex == 4) and ((configvalues[CONFIGIGNOREALLBRIGHTHNESSSETTINGS] == OFF))) then
    ...
    if (procedurestepindex == 18) then
        if (get(bankanglepos) ~= configvalues[CONFIGBANKANGLEMAX]) then
            ...
            elseif (configvalues[CONFIGVOICEADVICEONLY] == ON) then
                commandtableentry(ADVICE, "Set Maximum Bank Angle " .. tostring(configvalues[CONFIGBANKANGLEMAX]))
                procedurestepindex = procedurestepindex - 1
            end
        elseif ((configvalues[CONFIGVOICEADVICEONLY] == ON) and not proceduresteprepeat) then
            commandtableentry(ADVICE, "Maximum Bank Angle Checked")
        end
    end
...
function cockpitinit()
    ...
    if (procedurelock ~= NOPROCEDURE) then
        ...
        procedurelock = COCKPITINITPROCEDURE
    end
...
function aftertakeoffsteps()
    ...
    if ((procedurestepindex2 > 3) or procedureabort) then
    ...
    if (procedurestepindex2 == 1) then
        if (get(radioaltitude) > 200) then
            if (get(gearhandlepos) == GEARDOWN) then
                if (configvalues[CONFIGVOICEADVICEONLY] == ON) then
                    commandtableentry(ADVICE, "Set Gear Up")
                    procedurestepindex2 = procedurestepindex2 - 1
                else
                    set(gearhandlepos, GEARUP)
                    set(nosewheel, OFF)
                end
            elseif ((get(gearhandlepos) == GEARDOWN) and (configvalues[CONFIGVOICEADVICEONLY] == ON) and not proceduresteprepeat2) then
    ...
    if (procedurestepindex2 == 2) then
        ...
        elseif ((get(gearhandlepos) == GEAROFF) and (configvalues[CONFIGVOICEADVICEONLY] == ON) and not proceduresteprepeat2) then
    ...
    if (procedurestepindex2 == 3) then
        ...
        elseif ((configvalues[CONFIGVOICEADVICEONLY] == ON) and not proceduresteprepeat2) then
...
function altitudea10000steps()
    ...
    if ((procedurestepindex > 5) or procedureabort) then
...
function altitudea10000()
    ...
    if (procedurelock ~= NOPROCEDURE) then
        ...
        procedurelock = ALTITUDEA10000PROCEDURE
    end
...
function duringclimbsteps()
    ...
    if ((procedurestepindex2 > 13) or procedureabort) then
        duringclimbset = true
        procedurelock2 = NOPROCEDURE
        procedurestepindex2 = 1
        procedureabort = false
    ...
    if (procedurestepindex2 == 4) then
        if (configvalues[CONFIGVOICEADVICEONLY] ~= ON) then
            togglerwylights(OFF)
        end
    end

    if (procedurestepindex2 == 5) then
        if (configvalues[CONFIGVOICEADVICEONLY] ~= ON) then
            toggletaxilights(OFF)
        end
    end

    if (procedurestepindex2 == 6) then
        if (configvalues[CONFIGVOICEADVICEONLY] ~= ON) then
            if (configvalues[CONFIGTRANSPONDER] ~= 0) then
    ...
    if (procedurestepindex2 == 10) then
        if ((get(packlpos) == PACKOFF) or (get(packrpos) == PACKOFF)) then
            if (configvalues[CONFIGVOICEADVICEONLY] == ON) then
                commandtableentry(ADVICE, "Set Both Packs Auto")
                procedurestepindex2 = procedurestepindex2 - 1
    ...
    if (procedurestepindex2 == 11) then
        if (get(isolvalvepos)  == ISOLVALVEOPEN) then
            if (configvalues[CONFIGVOICEADVICEONLY] == ON) then
                commandtableentry(ADVICE, "Set Isolation Valve Auto")
                procedurestepindex2 = procedurestepindex2 - 1
    ...
    if (procedurestepindex2 == 12) then
        if (get(bleedairapupos) == ON) then
            if (configvalues[CONFIGVOICEADVICEONLY] == ON) then
                commandtableentry(ADVICE, "Switch A P U Bleed Air Off")
                procedurestepindex2 = procedurestepindex2 - 1
    ...
    if (procedurestepindex2 == 13) then
        if (get(apustarterpos) == ON) then
            if (configvalues[CONFIGVOICEADVICEONLY] == ON) then
                commandtableentry(ADVICE, "Switch A P U Off")
                procedurestepindex2 = procedurestepindex2 - 1
...
function duringclimb()
    ...
    if ((not duringclimbset) and (procedurelock2 == NOPROCEDURE)) then
        procedurelock2 = DURINGCLIMBPROCEDURE
    end

    if ((get(altitude) >= lowerairspacealt) and (not altitudea10000set) and (procedurelock == NOPROCEDURE)) then
        procedurelock = ALTITUDEA10000PROCEDURE
    end
...
function altitudeb10000steps()
    ...
    if ((procedurestepindex > 7) or procedureabort) then
    ...
    if (procedurestepindex == 5) then
        if (get(autobrakepos) == AUTOBRAKEOFF) then
            if (configvalues[CONFIGVOICEADVICEONLY] ~= ON) then
                setautobrake(AUTOBRAKE1)
            elseif (configvalues[CONFIGVOICEADVICEONLY] == ON) then
                commandtableentry(ADVICE, "Set Auto Brake")
                procedurestepindex = procedurestepindex - 1
            end
        elseif ((configvalues[CONFIGVOICEADVICEONLY] == ON) and not proceduresteprepeat) then
            commandtableentry(ADVICE, "Auto Brake Checked and Set")
        end  
    end
    ...
    if (procedurestepindex == 7) then
        if (configvalues[CONFIGVREF30SET] == ON) then
            ...
            elseif (configvalues[CONFIGVOICEADVICEONLY] == ON) then
                if (get(vref) ~= get(vref30)) then
                    commandtableentry(ADVICE, "Set V REF 30 " .. tostring(get(vref30)))
                    procedurestepindex = procedurestepindex - 1
                elseif ((configvalues[CONFIGVOICEADVICEONLY] == ON) and not proceduresteprepeat) then
                    commandtableentry(ADVICE, "V REF 30 Checked and Set")
                end
            end
        end
    end
...
function altitudeb10000()
    ...
    if (procedurelock ~= NOPROCEDURE) then
        ...
        procedurelock = ALTITUDEB10000PROCEDURE
    end
...
function radioaltitudeb2500steps()
    ...
    if ((procedurestepindex2 > 2) or procedureabort) then
    ...
    if (procedurestepindex2 == 1) then
        ...
        elseif (get(gearhandlepos) == GEARDOWN) then
            if ((configvalues[CONFIGVOICEADVICEONLY] == ON) and not proceduresteprepeat2) then
...
function radioaltitudeb1000steps()
    ...
    if ((procedurestepindex2 > 7) or prodedureabort) then
    ...
    if (procedurestepindex2 == 4) then
        if (get(missedappalt) ~= 0) then
            ...
            if (missedappalttmp ~= get(mcpaltitude)) then
                if (configvalues[CONFIGVOICEADVICEONLY] == ON) then
                    commandtableentry(ADVICE, "Set M C P Altitude " .. missedappalttmp)
                    procedurestepindex2 = procedurestepindex2 - 1
                else
                ...
                if ((configvalues[CONFIGVOICEADVICEONLY] == ON) and not proceduresteprepeat2) then
                    commandtableentry(ADVICE, "MCP Altitude Checked and " .. tostring(missedappalttmp))
                end
            end
        else
            if (configvalues[CONFIGVOICEADVICEONLY] == ON) then
    ...
    if (procedurestepindex2 == 5) then
        if ((get(desrwyheading) ~= 0) and (get(aphdgselstat) == OFF)) then
            if ((apphasils or (get(aploccapturedstat) == CAPTURED)) and (math.abs(roundnumber(get(desrwyheading)) - get(mcppilotcourse)) <= 1)) then
                missedapphdgtmp = get(mcppilotcourse)
            else
                missedapphdgtmp = roundnumber(get(desrwyheading))
            end
            if (missedapphdgtmp ~= get(mcpheading)) then
                if (configvalues[CONFIGVOICEADVICEONLY] == ON) then
                    commandtableentry(ADVICE, "Set M C P Heading " .. missedapphdgtmp)
                    procedurestepindex2 = procedurestepindex2 - 1
                else
                    set(mcpheading,  missedapphdgtmp)
                end
            else
                if ((configvalues[CONFIGVOICEADVICEONLY] == ON) and not proceduresteprepeat2) then
                    commandtableentry(ADVICE, "MCP Heading Checked and " .. tostring(missedapphdgtmp))
                end
            end
        elseif (get(desrwyheading) == 0) then
            if (configvalues[CONFIGVOICEADVICEONLY] == ON) then
...
function duringdescentsteps()
    ...
    if ((procedurestepindex2 > 4) or procedureabort) then
    ...
    if (procedurestepindex2 == 2) then
        if (configvalues[CONFIGSPDRESTR250] == ON) then
            if (configvalues[CONFIGVOICEADVICEONLY] == ON) then
                commandtableentry(ADVICE, "Set Speed Restriction below 10000 Feet to 250")
            else
                if not speedrstr250set then
                    if speedrstr250() then
                        speedrstr250set = true
                    else
                        procedurestepindex2 = procedurestepindex2 - 1
                    end
                end
    ...
    if (procedurestepindex2 == 4) then
        if (configvalues[CONFIGAUTOBARO] == ON) then
            if (get(altitude) < get(fmctranslvl)) then
                barodifftmp = math.abs(roundnumber((get(baropilot) - calclocalqnhinch()), 2))
                if ((get(barostd) == ON) or (barodifftmp > 0.01)) then
                    if (configvalues[CONFIGVOICEADVICEONLY] ~= ON) then
                        helpers.command_once("laminar/B738/EFIS_control/capt/push_button/std_press")
                        set(baropilot, calclocalqnhinch())
                    elseif (configvalues[CONFIGVOICEADVICEONLY] == ON) then
                        if (get(baroinhpa) == ON) then
                            commandtableentry(ADVICE, "Set Q N H " .. tostring(convqnhinchtopas(calclocalqnhinch())))
                        else
                            commandtableentry(ADVICE, "Set Q N H " .. tostring(calclocalqnhinch()))
                        end
                        procedurestepindex2 = procedurestepindex2 - 1
                    end
                elseif ((get(barostd) == OFF) and (barodifftmp <= 0.01)) then
                    if ((configvalues[CONFIGVOICEADVICEONLY] == ON) and not proceduresteprepeat2) then
                        if (get(baroinhpa) == ON) then
                            commandtableentry(ADVICE, "Q N H Checked and " .. tostring(convqnhinchtopas(calclocalqnhinch())))
                        else
                            commandtableentry(ADVICE, "Q N H Checked and " .. tostring(calclocalqnhinch()))
                        end
                    end
                end
            else
...
function duringdescent()
    ...
    if ((not duringdescentset) and (procedurelock2 == NOPROCEDURE)) then
        procedurelock2 = DURINGDESCENTPROCEDURE
    end

    if ((get(altitude) < lowerairspacealt) and (not altitudeb10000set) and (procedurelock == NOPROCEDURE)) then
        procedurelock = ALTITUDEB10000PROCEDURE
    end

    if ((get(radioaltitude) < 2500) and (not radioaltitude2500set) and (procedurelock2 == NOPROCEDURE)) then
        procedurelock2 = RADIOALTITUDEB2500PROCEDURE
    end

    if ((get(radioaltitude) < 1000) and (not radioaltitude1000set)  and (procedurelock2 == NOPROCEDURE)) then
        procedurelock2 = RADIOALTITUDEB1000PROCEDURE
    end
...
function afterlanding()
    ...
    if (procedurelock ~= NOPROCEDURE) then
        ...
        procedurelock = AFTERLANDINGPROCEDURE
    end
    ...
    if (procedurestepindex == 11) then
        if (configvalues[CONFIGVOICEADVICEONLY] == ON) then
            if (get(aplnavstat) ~= ON) then
                ...
                procedurestepindex = procedurestepindex - 1
            elseif not proceduresteprepeat then
...
function beforetaxi()
    ...
    if (procedurelock ~= NOPROCEDURE) then
        ...
        procedurelock = BEFORETAXIPROCEDURE
    end
...
function beforetakeoffsteps()
    ...
    if ((procedurestepindex > 12) or procedureabort) then
    ...
    if (procedurestepindex == 7) then
        if (get(autobrakepos)  ~= AUTOBRAKERTO) then
            ...
            elseif (configvalues[CONFIGVOICEADVICEONLY] == ON) then
                commandtableentry(ADVICE, "Set Autobrake R T O")
                procedurestepindex = procedurestepindex - 1
            end
        elseif ((configvalues[CONFIGVOICEADVICEONLY] == ON) and not proceduresteprepeat) then
            commandtableentry(ADVICE, "Autobrake Checked and R T O")
        end
    end
    ...
    if (procedurestepindex == 8) then
        if (configvalues[CONFIGVOICEADVICEONLY] == ON) then
            ...
            if (get(mcpheading) ~= headingrounded) then
                commandtableentry(ADVICE, "Set M C P Heading" .. tostring(headingrounded))
                procedurestepindex = procedurestepindex - 1
            elseif not proceduresteprepeat then
                commandtableentry(ADVICE, "M C P Heading Checked" .. tostring(headingrounded))
            end
        end
    end
...
function beforetakeoff()
    ...
    if (procedurelock ~= NOPROCEDURE) then
        ...
        procedurelock = BEFORETAKEOFFPROCEDURE
    end
...
function autofunctions()
    ...
    if (get(airgroundsensor) == ON)  then -- aircraft on the ground
        ...
        if (flightstate == 0) then
            if ((not beforetaxiset) and (get(taxilight) ~= OFF) and enginesrunning(BOTH) and (get(groundspeed) < 45) and (procedurelock == NOPROCEDURE)) then
                procedurelock = BEFORETAXIPROCEDURE
            end

            if (beforetaxiset and (not beforetakeoffset) and (procedurelock == NOPROCEDURE)) then
            ...
            if ((not afterlandingset) and (get(groundspeed) < 45) and (procedurelock == NOPROCEDURE)) then
            ...
            if ((get(parkingbrakepos) == ON) and (flightstate >= 5) and afterlandingset and (not atparkingpositionset) and (procedurelock == NOPROCEDURE)) then
                flightstate = 6
                procedurelock = ATPARKINGPOSITIONPROCEDURE
            end
        end
        ...
        if ((flightstate == 1) and (not aftertakeoffset)  and (procedurelock2 == NOPROCEDURE)) then
            procedurelock2 = AFTERTAKEOFFPROCEDURE
...
function voicereadback()
    ...
    if (get(mcpheading) ~= mcpheadingtemp) then
        if (get(mcpheading) ~= mcpheadingtemp2) then
            ...
            commandtableentry(TEXT, "M C P Heading " .. tostring(get(mcpheading)))
        end
    end
    ...
    if (get(mcpaltitude) ~= mcpaltitudetemp) then
        if (get(mcpaltitude) ~= mcpaltitudetemp2) then
            ...
            if (get(mcpaltitude) == get(fmccruisealt)) then
                commandtableentry(TEXT, "M C P set to Cruise Altitude " .. tostring(get(mcpaltitude)))
            else
                commandtableentry(TEXT, "M C P Altitude " .. tostring(get(mcpaltitude)))
            end
        end
    end
    ...
    if (get(mcppilotcourse) ~= mcppilotcoursetemp) then
        if (get(mcppilotcourse) ~= mcppilotcoursetemp2) then
            ...
            commandtableentry(TEXT, "M C P Pilot Course " .. tostring(get(mcppilotcourse)))
        end
    end
    ...
    if (get(mcpcopilotcourse) ~= mcpcopilotcoursetemp) then
        if (get(mcpcopilotcourse) ~= mcpcopilotcoursetemp2) then
            ...
            commandtableentry(TEXT, "M C P Copilot Course " .. tostring(get(mcpcopilotcourse)))
        end
    end
    ...
    if ((get(baropilot) ~= baropilottemp) or (get(barostd) ~= barostdtemp)) then
        if (get(baropilot) ~= baropilottemp2) then
            ...
            if ((get(barostd) == ON) and (get(barostd) ~= barostdtemp)) then
                ...
                if (get(baroinhpa) == ON) then
                    commandtableentry(TEXT, "Q N H " .. tostring(convqnhinchtopas(get(baropilot))))
                else
    ...
    if (get(mmrinstalled) == ON) then
        ...
        if ((get(nav1freq) ~= nav1freqtemp) or (get(nav2freq) ~= nav2freqtemp)) then
            if (get(nav1freq) == get(nav2freq)) then
                commandtableentry(TEXT, "Both N A V " .. tostring(get(nav1freq) / 100))

                nav1freqtemp = get(nav1freq)
                nav2freqtemp = get(nav2freq)
            else
                if (get(nav1freq) ~= nav1freqtemp) then
                    commandtableentry(TEXT, "N A V 1 " .. tostring(get(nav1freq) / 100))

                    nav1freqtemp = get(nav1freq)
                end

                if (get(nav2freq) ~= nav2freqtemp) then
                    commandtableentry(TEXT, "N A V 2 " .. tostring(get(nav2freq) / 100))
    ...
    if ((get(announcsourceoff1) ~= announcsourceoff1temp) or (get(announcsourceoff2) ~= announcsourceoff2temp)) then
        if (get(apurunning) == ON) then
    ...
    if (get(bankanglepos) ~= bankanglepostemp) then
        if (get(bankanglepos) == BANKANGLEMIN) then
            commandtableentry(TEXT, "Bank Angle Minimum")
        elseif (get(bankanglepos) == BANKANGLE15) then
            commandtableentry(TEXT, "Bank Angle 15")
        elseif (get(bankanglepos) == BANKANGLE20) then
            commandtableentry(TEXT, "Bank Angle 20")
        elseif (get(bankanglepos) == BANKANGLE25) then
            commandtableentry(TEXT, "Bank Angle 25")
        elseif (get(bankanglepos) == BANKANGLEMAX) then
            commandtableentry(TEXT, "Bank Angle Maximum")
        end
    ...
    if (get(autobrakepos) ~= autobrakepostemp) then
        if (get(autobrakepos) == AUTOBRAKERTO) then
            commandtableentry(TEXT, "Autobrake R T O")
        elseif (get(autobrakepos) == AUTOBRAKEOFF) then
            commandtableentry(TEXT, "Autobrake Off")
        elseif (get(autobrakepos) == AUTOBRAKE1) then
            commandtableentry(TEXT, "Autobrake 1")
        elseif (get(autobrakepos) == AUTOBRAKE2) then
            commandtableentry(TEXT, "Autobrake 2")
        elseif (get(autobrakepos) == AUTOBRAKE3) then
            commandtableentry(TEXT, "Autobrake 3")
        elseif (get(autobrakepos) == AUTOBRAKEMAX) then
            commandtableentry(TEXT, "Autobrake Maximum")
        end
    ...
    if (get(autobrakedisarm) ~= autobrakedisarmtemp) then
        if (get(autobrakedisarm) ~= autobrakedisarmtemp2) then
            ...
            if (get(autobrakedisarm) == ON) then
                commandtableentry(TEXT, "Autobrake Disarmed")
            end
...
function ongoingtasks()
    ...
    if ((procedurelock == NOPROCEDURE) and (configvalues[CONFIGVOICEADVICEONLY] == ON)  and (get(airgroundsensor) == ON)) then
        if ((get(starter1pos) == GROUND) and (get(eng1n2percent) > 25) and (get(mixture1pos) == OFF)) then 
            commandtableentry(ADVICE, "Engine 1 N 2 at 25 Percent")
        end
        if ((get(starter2pos) == GROUND) and (get(eng2n2percent) > 25) and (get(mixture2pos) == OFF)) then 
            commandtableentry(ADVICE, "Engine 2 N 2 at 25 Percent")
        end
        if ((get(atarmpos) == ARMED) and (get(atn1stat) == OFF) and (get(groundspeed) < 45) and (get(eng1n1percent) > 40) and (get(eng1n1percent) > 40)) then 
            commandtableentry(ADVICE, "Both Engine N 1 at 40 Percent")
        end
        if ((get(apustarterpos) == ON) and (get(apugenoffbus) ~= OFF) and (get(gen1pos) == OFF) and (get(gen1pos) == OFF) and (not((get(apupowerbus1) == ON) and (get(announcsourceoff1) == OFF)) or not((get(apupowerbus2) == ON) and (get(announcsourceoff2) == OFF)))) then
            commandtableentry(ADVICE, "A P U Running")
        end
    end
    ...
    if (ongoingtaskstepindex == 2) then
        if ( (flightstate < 3) and (get(fmccruisealt) ~= 0) and (get(fmccruisealt) ~= 20000) and (get(cabincruisealt)  ~= get(fmccruisealt))) then
            ...
            elseif (configvalues[CONFIGVOICEADVICEONLY] == ON) then 
                commandtableentry(ADVICE, "Set Cabin Cruise Alitude " .. tostring(get(fmccruisealt)))
                ongoingtaskstepindex = ongoingtaskstepindex - 1
            end
        end
    end
    ...
    if (ongoingtaskstepindex == 3) then
        if ((configvalues[CONFIGAUTOFUNCTIONS] == ON) and (configvalues[CONFIGVOICEADVICEONLY] ~= ON) and (flightstate < 4)) then
            set(cabinlandingalt, roundnumber(get(desrwyalt) / 50) * 50)
        end
    end
    ...
    if (((get(airgroundsensor) == ON) and (procedurelock == NOPROCEDURE) and (get(battery) == ON) and (get(mainbus) ~= OFF) and (flightstate == 0) and (get(taxilight) == OFF))) then

        if (ongoingtaskstepindex == 6) then
            if (configvalues[CONFIGAUTOBARO] == ON) then
                barodifftmp = math.abs(roundnumber((get(baropilot) - calclocalqnhinch()), 2))
                if (barodifftmp > 0.01) then
                    if ((configvalues[CONFIGAUTOFUNCTIONS] == ON) and (configvalues[CONFIGVOICEADVICEONLY] ~= ON)) then
                        set(baropilot, calclocalqnhinch())
                    elseif (configvalues[CONFIGVOICEADVICEONLY] == ON) then
                        if (get(baroinhpa) == ON) then
        ...
        if (ongoingtaskstepindex == 7) then
            if (get(trimcalc) > 0) and (get(trimcalc) ~= gettrim()) then
                if (((configvalues[CONFIGAUTOFUNCTIONS] == ON) and (configvalues[CONFIGVOICEADVICEONLY] ~= ON))) then
                    settotrim()
        ...
        if (ongoingtaskstepindex == 8) then
            if ((get(v2speed) > 0) and (get(v2speed) ~= get(mcpspeed))) then
                if ((configvalues[CONFIGAUTOFUNCTIONS] == ON) and (configvalues[CONFIGVOICEADVICEONLY] ~= ON)) then
                    set(mcpspeed, get(v2speed))
                elseif (configvalues[CONFIGVOICEADVICEONLY] == ON) then
                    commandtableentry(ADVICE, "Set M C P Speed " .. tostring(get(v2speed)))
                    ongoingtaskstepindex = ongoingtaskstepindex - 1         
                end
            end
        end
        ...
        if (ongoingtaskstepindex == 9) then
            headingrounded = roundnumber(get(deprwyheading))
            if ((headingrounded ~= 0) and (headingrounded  ~= get(mcpheading))) then
                if ((configvalues[CONFIGAUTOFUNCTIONS] == ON) and (configvalues[CONFIGVOICEADVICEONLY] ~= ON)) then
...
--------------------------------------------------------------------------------------------------------------
-- procedureloop() function

function procedureloop()

    if (procedurelock ~= NOPROCEDURE) then
        if (procedurelock == COCKPITINITPROCEDURE) then
            cockpitinitsteps()
        elseif (procedurelock == COLDANDDARKPROCEDURE) then
            coldanddarksteps()
        elseif (procedurelock == APUSTARTUPPROCEDURE) then
            apustartupsteps()
        elseif (procedurelock == ENGINESTARTPROCEDURE) then
            enginestartsteps()
        elseif (procedurelock == BEFORETAXIPROCEDURE) then
            beforetaxisteps()
        elseif (procedurelock == BEFORETAKEOFFPROCEDURE) then
            beforetakeoffsteps() 
        elseif (procedurelock == AFTERLANDINGPROCEDURE) then
            afterlandingsteps() 
        elseif (procedurelock == TURNAROUNDENGINESHUTDOWNPROCEDURE) then
            engineshutdownsteps()
        elseif (procedurelock == FINALENGINESHUTDOWNPROCEDURE) then
            engineshutdownsteps()
        elseif (procedurelock == SHUTDOWNPROCEDURE) then
            shutdownsteps()
        elseif (procedurelock == TESTPROCEDURE) then
            teststeps()
        elseif (procedurelock == ALTITUDEA10000PROCEDURE) then
            altitudea10000steps()
        elseif (procedurelock == ALTITUDEB10000PROCEDURE) then
            altitudeb10000steps()
        elseif (procedurelock == ATPARKINGPOSITIONPROCEDURE) then
            atparkingpositionsteps()
        elseif (procedurelock == SETILSPROCEDURE) then
            if setilssteps() then
                procedurelock = NOPROCEDURE
            end
        elseif (procedurelock == SETVREF30PROCEDURE) then
            if setvref() then
                procedurelock = NOPROCEDURE
            end
        elseif (procedurelock == SETSPDRESTR250PROCEDURE) then
            if speedrstr250() then
                procedurelock = NOPROCEDURE
            end
        else
        ...
        if (procedurestepindex == procedurestepindexprevious) then
            proceduresteprepeat = true
        else
            proceduresteprepeat = false
            procedurestepindexprevious = procedurestepindex
        end
...
--------------------------------------------------------------------------------------------------------------
-- procedureloop2() function

function procedureloop2()

    if (procedurelock2 ~= NOPROCEDURE) then
        if (procedurelock2 == AFTERTAKEOFFPROCEDURE) then
            aftertakeoffsteps()
        elseif (procedurelock2 == DURINGCLIMBPROCEDURE) then
            duringclimbsteps()
        elseif (procedurelock2 == DURINGDESCENTPROCEDURE) then
            duringdescentsteps()
        elseif (procedurelock2 == RADIOALTITUDEB2500PROCEDURE) then
            radioaltitudeb2500steps()
        elseif (procedurelock2 == RADIOALTITUDEB1000PROCEDURE) then
        ...
        if (procedurestepindex2 == procedurestepindexprevious2) then
            proceduresteprepeat2 = true
        else
            proceduresteprepeat2 = false
            procedurestepindexprevious2 = procedurestepindex2
        end
...
function P.do_yal()
    ...
    sasl.logDebug("PROCEDURELOOP: PROCEDURELOCK ".. procedurelock .. " PROCEDURESTEPINDEX " .. procedurestepindex)
    sasl.logDebug("PROCEDURELOOP2: PROCEDURELOCK2 ".. procedurelock2 .. " PROCEDURESTEPINDEX2 " .. procedurestepindex2)
...
menu_speed_restrict = sasl.appendMenuItem(P.menu_main, "Set Speed Restriction 250", speedrstr250proc)
...
function P.enableMenus()
    ...
    sasl.enableMenuItem(P.menu_main , menu_speed_restrict , enable)
