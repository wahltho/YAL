...
local function route_initial_runway_progress(route, data, profile)
    if not route or not data or not profile then
        return nil
    end
    local path = route.path
    if not path or #path < 2 or not data.nodes then
        return nil
    end
    local first_idx = nil
    local last_idx = nil
    for i = 1, (#path - 1) do
        local from_id = path[i]
        local to_id = path[i + 1]
        local label = get_edge_label(data, from_id, to_id)
        local is_rwy = (label and is_runway_label(label))
            or (data.runway_nodes and data.runway_nodes[from_id] and data.runway_nodes[to_id])
        if is_rwy then
            if not first_idx then
                first_idx = i
            end
            last_idx = i + 1
        elseif first_idx then
            break
        end
    end
    if not first_idx or not last_idx or last_idx <= first_idx then
        return nil
    end
    local first_node = data.nodes[path[first_idx]]
    local last_node = data.nodes[path[last_idx]]
    if not first_node or not last_node then
        return nil
    end
    local start_along = compute_along_perp(profile, first_node)
    local end_along = compute_along_perp(profile, last_node)
    if start_along == nil or end_along == nil then
        return nil
    end
    return {
        start_along = start_along,
        end_along = end_along,
        delta = end_along - start_along,
        first_node_id = path[first_idx],
        last_node_id = path[last_idx]
    }
end
...
evaluate_arrival_route_validity = function(route, data, aircraft, landing_profile, arr_exit_id, drift_meters, off_runway, backtrack_required, active_context)
    ...
    if on_runway and landing_profile then
        local runway_progress = route_initial_runway_progress(route, route_data, landing_profile)
        info.runway_progress = runway_progress
        if runway_progress then
            local reverse_limit = math.max(30, drift_meters * 0.6)
            if runway_progress.delta < -reverse_limit then
                info.valid = false
                info.severe = true
                info.reason = "runway-backward"
                return info
            end
        end
    end
...
local function choose_or_keep_arrival_exit(comp, landing_profile, data, proposed_exit_id, proposed_backtrack, aircraft, off_runway, active_context, start_lat, start_lon, log_taxi)
    ...
    if active_context and off_runway == false and comp._route and comp._arrExitId and exit_id and exit_id ~= comp._arrExitId then
        ...
        if keep_committed then
            local committed_exit = comp._arrExitId
            exit_id = committed_exit
            exit_along = comp._arrExitAlong
            backtrack_required = comp._arrBacktrackRequired and true or false
            start_lat, start_lon = apply_exit_start(committed_exit)
            log_taxi(
                string.format(
                    "TaxiRoute: ARR exit commit keep=%s new=%s",
                    tostring(committed_exit),
                    tostring(proposed_exit_id)
                )
            )
    ...
    if not in_edit then
        ...
        if comp.mode == 0 and not comp._depTaxiCompleteAnnounced and comp._runwayName and comp._runwayName ~= "" then
            ...
            if onGround and comp.yal and comp.yal.aircraftonrwy then
                ...
                if (not onRunway) and (not nearRunwayEntry) and comp._depThresholdState
                    and comp._route and comp._route.path and comp._route.data
                    and #comp._route.path >= 2 and aircraft and aircraft.east ~= nil and aircraft.north ~= nil then
                    local path = comp._route.path
                    local data = comp._route.data
                    local st = comp._depThresholdState
                    local gate = (C.depThresholdGateMeters or 45)
                    local seg_idx = U.find_nearest_segment(data, path, aircraft.east, aircraft.north)
                    local path_len = #path
                    local route_near_end = seg_idx and seg_idx >= math.max(1, path_len - 2)
                    local route_has_dep_runway = false
                    local dep_runway = normalize_runway_name(comp._runwayName or "")
                    if route_near_end and dep_runway ~= "" then
                        for i = math.max(1, path_len - 4), (path_len - 1) do
                            local lbl = get_edge_label(data, path[i], path[i + 1]) or ""
                            if lbl ~= "" and is_runway_label(lbl) then
                                local pair = normalize_runway_pair_label(lbl)
                                local a, b = pair:match("^([^/]+)/([^/]+)$")
                                if a == dep_runway or b == dep_runway then
                                    route_has_dep_runway = true
                                    break
                                end
                            end
                        end
                    end
                    if route_near_end and (not route_has_dep_runway)
                        and st.dist and st.dist <= math.max(70, gate * 1.5)
                        and st.perp and st.corridor and st.perp <= math.max(60, st.corridor * 2.5) then
                        nearRunwayEntry = true
                    end
                end
